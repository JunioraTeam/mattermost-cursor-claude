...
from .messages import last_user_message, render_conversation, system_instructions
...
async def handle_chat_completions(
    ...
    *,
    ...
    # The plugin sends no stable "user" and resends the whole thread each call, so
    # the bot stays stateless: a fresh agent per request, fed the full transcript.
    user_id = (body.get("user") or "").strip() or "default"
    username = user_id if user_id != "default" else None
    prompt = augment_user_message_for_plugin(
        env, render_conversation(messages), system_instructions(messages)
    )

    await history.record_user_event(
        userId=user_id, username=username, type="message", preview=user_text
    )
    history_id = await history.start_run(
        source="openai", userId=user_id, username=username, messagePreview=user_text
    )

    # Fresh agent per request; disposed in finally so it never leaks.
    agent, mcp_servers = await sessions.create_agent_once()
    try:
        try:
